...
| B4 | not measurable yet — instrumentation task S3.2 | Proof-reach rate — sessions firing any of {repo click, demo click, `case_study_read`} ÷ sessions on `/projects/*` | `case_study_read` is absent from the PostHog event schema (verified via `read-data-schema` kind=events on 2026-09-18), so the metric cannot be computed as defined. No `demo_click`-style event exists in the schema either. **Sub-signals currently measurable:** repo-click events only (`project_github_*` + `github_footer`) — this is the B3 numerator, 5 sessions. The `demo click` and `case_study_read` sub-signals are not measurable until instrumented. Becomes computable after S3.2 adds `case_study_read` at 75% scroll depth on `/projects/*`. | 2026-09-18 | `[V]` (for the "not measurable" finding) |
