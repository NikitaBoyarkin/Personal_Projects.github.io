...
- Pages that are intentionally RU-only (no EN mirror): `contact`, `cv`, `guides`, `notes`, `now`, `writing`, `404`. Do not pass `counterpartHref` on these — `LangSwitch` is hidden and no hreflang alternates are emitted.
